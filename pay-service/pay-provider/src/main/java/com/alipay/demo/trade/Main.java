...
//        main.test_trade_precreate();
    ...
    // 测试当面付2.0生成支付二维码
    public String test_trade_precreate(PayRequest payRequest) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        String outTradeNo = "tradeprecreate" +payRequest.getOrderId();

        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = "CSMALL当面付扫码消费";

        // (必填) 订单总金额，单位为元，不能超过1亿元
        // 如果同时传入了【打折金额】,【不可打折金额】,【订单总金额】三者,则必须满足如下条件:【订单总金额】=【打折金额】+【不可打折金额】
        String totalAmount = String.valueOf(payRequest.getMoney());
        ...
        List<GoodsDetail> goodsDetailList = new ArrayList<GoodsDetail>();
        // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        GoodsDetail goods1 = GoodsDetail.newInstance("1", "小短裙", 10, 1);
        // 创建好一个商品后添加至商品明细列表
        goodsDetailList.add(goods1);
...
//        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                filePath = String.format("F:\\Documents\\Works@cskaoyan\\09 Project3\\qr\\qr-%s.png", response.getOutTradeNo());
            ...
            case FAILED:
                log.error("支付宝预下单失败!!!");
                errorMessage="支付宝预下单失败";
                break;

            case UNKNOWN:
                log.error("系统异常，预下单状态未知!!!");
                errorMessage="系统异常，预下单状态未知!!!";
                break;

            default:
                log.error("不支持的交易状态，交易返回异常!!!");
                errorMessage="不支持的交易状态，交易返回异常!!!";
                break;
        }
        if (!StringUtils.isBlank(filePath)){
            return String.format("qr-%s.png",outTradeNo);
